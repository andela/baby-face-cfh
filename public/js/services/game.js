angular.module('mean.system')
  .factory('game', [
    ...
    ($rootScope, socket, $timeout, $window, $http) => {
      ...
      socket.on('saveGameSession', (gameSessionData) => {
        const { localStorage: { token } } = $window;
        if (game.state === 'game ended' && token) {
          $http.post(`/api/games/${game.gameID}/start`, gameSessionData, {
            headers: {
